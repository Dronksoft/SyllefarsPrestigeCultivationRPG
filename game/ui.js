...
function openPanel(id){
  PANELS.forEach(p=>{
    ...
    if(!el)return;
    el.classList[p===id?'add':'remove']('panel-open');
    if(p==='prestige'&&p===id){ el.style.transform='translateX(-50%) scale(1) translateY(0)'; }
    else if(p==='prestige'){ el.style.transform='translateX(-50%) scale(0.96) translateY(8px)'; }
  });
}
...
function startDrag(e,panelId){
  ...
  const onMove=ev=>{p.style.left=(ev.clientX-ox)+'px';p.style.top=(ev.clientY-oy)+'px';p.style.right='auto';if(panelId!=='panel-prestige')p.style.transform='none';else p.style.transform='scale(1) translateY(0)';};
...
function setBar(type,cur,max){
  ...
  v.textContent=type==='xp'?cur.toLocaleString()+'/'+max.toLocaleString():cur+'/'+max;
}
function updateHUD(){
  const p=window.GameState.player;
  // Support both step1 field names (currentHP/maxHP) and step3 stub names (hp/maxHp)
...
function showLootPop(items){
  ...
  items.forEach(item=>{
    ...
    card.querySelector('.loot-dismiss').addEventListener('click',e=>{e.stopPropagation();clearTimeout(card._t);card.classList.add('fading');setTimeout(()=>card.remove(),500)});
    card.addEventListener('click',e=>{if(e.target.classList.contains('loot-dismiss'))return;window.GameEventBus.emit('loot:pickup',{itemId:item.id});card.classList.add('fading');setTimeout(()=>card.remove(),500)});
    panel.appendChild(card);
    card._t=setTimeout(()=>{card.classList.add('fading');setTimeout(()=>card.remove(),500)},8000);
  });
}
...
function renderBooks(){
  ...
  document.getElementById('book-list').innerHTML=list.map(b=>{
    ...
    return `<div class="book-entry ${ac}" data-id="${b.id}">
      ...
      <button class="${btn}" ${(!b.active&&!bookUnlocked)?'disabled':''} title="${(!b.active&&!bookUnlocked)?'Available on Prestige':''}" onclick="event.stopPropagation();activateBook('${b.id}','${b.type}')">
...
function activateBook(id,type){
  ...
  if(b){b.active=true;window.GameEventBus.emit('book:activated',{bookId:id});document.getElementById('active-book-badge').textContent=b.name;}
  renderBooks();
}
...
function renderCores(){
  document.getElementById('core-grid').innerHTML=CORES.map(c=>{
    const prog=c.locked?Math.min(100,Math.floor((window.GameState.player.level/c.lvl)*100)):100;
    return `<div class="core-card${c.locked?' locked-core':''}${c.active?' active-core':''}${selCoreId===c.id&&!c.locked&&!c.active?' selected-core':''}"
      ...
      ${c.locked?`<div class="core-progress"><div class="core-progress-track"><div class="core-progress-fill" style="width:${prog}%"></div></div><div class="core-progress-text">Lv.${window.GameState.player.level}/${c.lvl} required</div></div>`:''}
    </div>`;
  }).join('');
  updateCoreSwapUI();
}
...
function updateCoreSwapUI(){
  ...
  if(inCombat){btn.disabled=true;s.className='core-swap-status locked';st.textContent='Cannot swap during combat';}
  else if(cdCountdown>0){btn.disabled=true;s.className='core-swap-status cooling';st.textContent='Cooldown: '+cdCountdown+'s';}
  else{btn.disabled=false;s.className='core-swap-status ready';st.textContent='Core swap available';}
}
...
function renderStash(){
  ...
  for(let i=0;i<32;i++){
    ...
    html+=item?`<div class="stash-cell has-item rarity-${item.rarity}" title="Item" onclick="UI.equipItem('${item.id}')" onmouseenter="showTT(event,'${item.id}')" onmouseleave="hideTT()">${item.icon}</div>`:`<div class="stash-cell"></div>`;
  }
  g.innerHTML=html;
}
...
function selectDiff(d){
  ...
  document.querySelectorAll('.diff-radio-label').forEach(l=>{
    ...
    if(!ld)return;
    l.className='diff-radio-label'+(l.classList.contains('locked-diff')?' locked-diff':ld===d?' checked-'+d:'');
  });
}
...
function confirmPrestige(){
  ...
  window.GameEventBus.emit('ui:prestigeConfirmed',{activeMartialBook:mBook,activeSoulBook:sBook,activeCore:core,difficulty:selectedDiff});
  // Fanfare
  const p=window.GameState.player;
  document.getElementById('fanfare-subtitle').textContent='Prestige ×'+(p.prestige+1)+' — '+selectedDiff.charAt(0).toUpperCase()+selectedDiff.slice(1)+' Difficulty';
  const f=document.getElementById('prestige-fanfare');f.classList.add('active');
  setTimeout(()=>f.classList.remove('active'),4000);
  // State reset is handled by PrestigeManager.executePrestige() triggered via integration.js
}
...
window.UI={
  ...
  swapCore(){
    ...
    document.getElementById('active-core-badge').textContent=c.name;
    renderCores();
    window.GameEventBus.emit('core:fused',{coreId:selCoreId});
    document.querySelectorAll('.core-card.active-core').forEach(el=>{el.classList.add('core-fuse-anim');setTimeout(()=>el.classList.remove('core-fuse-anim'),700);});
    cdCountdown=30;if(cdTimer)clearInterval(cdTimer);
    cdTimer=setInterval(()=>{cdCountdown--;if(cdCountdown<=0){cdCountdown=0;clearInterval(cdTimer);}updateCoreSwapUI();},1000);
    updateCoreSwapUI();
  },
...
(function setupListeners(){
  ...
  bus.on('player:damaged',d=>{
    // State already updated by engine.js; just refresh UI + visual feedback
    updateHUD();
    const t=document.getElementById('bar-hp-track');if(t){t.classList.add('bar-flashing');setTimeout(()=>t.classList.remove('bar-flashing'),900);}
    spawnFCT(d?.x||window.innerWidth*0.22,d?.y||window.innerHeight*0.45,'-'+(d?.amount||'??'),'fct-damage-taken');
  });
  bus.on('player:healed',d=>{
    updateHUD();
    spawnFCT(d?.x||window.innerWidth*0.22,d?.y||window.innerHeight*0.45,'+'+(d?.amount||0),'fct-heal');
  });
  bus.on('player:levelup',d=>{
    // Level/XP state already updated by engine.js
    updateHUD();
  ...
  bus.on('xp:gained',d=>{
    updateHUD();
    spawnFCT(d?.x||window.innerWidth*0.5,d?.y||window.innerHeight*0.5,'+'+(d?.amount||0)+' XP','fct-xp');
  });
  bus.on('loot:dropped',d=>showLootPop(d?.items||[]));
  bus.on('book:learned',()=>renderBooks());
  bus.on('book:activated',d=>{const b=BOOKS.find(x=>x.id===d?.bookId);if(b){BOOKS.forEach(x=>{if(x.type===b.type)x.active=false;});b.active=true;document.getElementById('active-book-badge').textContent=b.name;renderBooks();}});
  bus.on('book:activationUnlocked',()=>{bookUnlocked=true;renderBooks();});
  bus.on('core:fused',d=>{const c=CORES.find(x=>x.id===d?.coreId);if(c){CORES.forEach(x=>x.active=false);c.active=true;document.getElementById('active-core-badge').textContent=c.name;renderCores();}});
  bus.on('prestige:available',()=>{window.GameState.prestigeReady=true;document.getElementById('prestige-indicator').classList.add('visible');});
  bus.on('prestige:complete',()=>{window.GameState.prestigeReady=false;document.getElementById('prestige-indicator').classList.remove('visible');PANELS.forEach(p=>closePanel(p));updateHUD();});
  bus.on('area:entered',d=>{document.getElementById('area-name-text').textContent=d?.name||(window.WorldData?.currentArea?.name)||'Unknown Region';});
  bus.on('combat:started',()=>{inCombat=true;updateCoreSwapUI();});
  bus.on('combat:ended',()=>{inCombat=false;updateCoreSwapUI();});
})();

/* ═════════════════════════════════════════════
   SKILL COOLDOWN DEMO TICKER
═════════════════════════════════════════════ */
let cd1=3,cd1Max=5;
setInterval(()=>{
  if(cd1>0){
    cd1=Math.max(0,cd1-0.05);
    const pct=(cd1/cd1Max)*100;
    const sw=document.getElementById('cd-sweep-1');
    const ov=document.getElementById('cd-overlay-1');
    if(sw){sw.style.setProperty('--cd-pct',pct.toFixed(1)+'%');sw.className='cooldown-sweep'+(cd1>0?' active':'');}
    if(ov)ov.textContent=cd1>0?Math.ceil(cd1):'';
  }
},50);
...
function demoEvent(type){
  ...
  switch(type){
    ...
    case 'loot:dropped': window.GameEventBus.emit('loot:dropped',{items:[{id:'l'+Date.now(),icon:'⚔️',name:'Ember Blade',rarity:'magic'},{id:'l'+(Date.now()+1),icon:'💎',name:'Void Shard Core',rarity:'unique'}]}); break;
...
window.GameData=window.GameData||{books:BOOKS,cores:CORES,origins:[{id:'iron_path',name:'Iron Path',lore:'Born where stone meets sky.'}]};